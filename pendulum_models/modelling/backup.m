function main()
    % Main function for Cart-Pendulum System Simulation (cart drives pendulum)
    ...
    t = setup_simulation(params);
    
    % Define cart motion
    [x_c, x_c_dot, x_c_ddot] = define_cart_motion(t, params);
    
    % Simulate pendulum using angular equation of motion
    [theta, theta_dot] = simulate_pendulum_angular(t, x_c_ddot, params);
    
    % Calculate pendulum positions
    [x_pendulum, y_pendulum] = calculate_positions(theta, x_c, params);
    
    % Plot results
    plot_results(t, theta, theta_dot, x_c, x_pendulum, y_pendulum, params);
    
    % Run animation
    animate_system(t, theta, x_c, x_pendulum, y_pendulum, params);
...
function params = get_parameters()
    ...
    params.L = 1.0;            % pendulum length (m)
    params.m_p = 0.1;          % pendulum mass (kg)
    params.m_c = 1.0;          % cart mass (kg) - for visualization only
    params.b = 0.01;           % damping coefficient
    
    % Simulation parameters
    params.dt = 0.01;          % time step (s)
    params.t_end = 10;         % simulation time (s)
    
    % Initial conditions
    params.theta_0 = pi/6;     % initial angle (rad)
    params.theta_dot_0 = 0;    % initial angular velocity (rad/s)
    params.x_c_0 = 0;          % initial cart position (m)
    
    % Animation parameters
    params.anim_skip = 5;      % animate every nth frame
    params.trail_length = 100; % length of pendulum trail
end

function t = setup_simulation(params)
    % Set up time vector for simulation
    t = 0:params.dt:params.t_end;
end

function [x_c, x_c_dot, x_c_ddot] = define_cart_motion(t, params)
...
function [theta, theta_dot] = simulate_pendulum_angular(t, x_c_ddot, params)
    % Simulate pendulum using angular equation of motion (cart drives pendulum)
    n_steps = length(t);
    theta = zeros(n_steps, 1);
    theta_dot = zeros(n_steps, 1);
    theta(1) = params.theta_0;
    theta_dot(1) = params.theta_dot_0;
    dt = params.dt;
    
    for i = 1:n_steps-1
        % Equation: θ'' + (b/mL^2) θ' + (g/L) sinθ = -x_c_ddot/L * cosθ
        theta_ddot = - (params.b/(params.m_p*params.L^2)) * theta_dot(i) ...
                     - (params.g/params.L) * sin(theta(i)) ...
                     - (x_c_ddot(i)/params.L) * cos(theta(i));
...
function [x_pendulum, y_pendulum] = calculate_positions(theta, x_c, params)
    % Calculate pendulum bob positions
    x_pendulum = x_c(:) + params.L * sin(theta(:));
    y_pendulum = -params.L * cos(theta(:));
end
...
function draw_pendulum(x_cart, x_pend, y_pend)
    % Draw the pendulum rod and mass
    % Pendulum rod
