...
% 
% These equations are represented in Simulink(R) with commonly used blocks.
...
%%
% Examine the zeros (if any) of the linearized plant.
tzero(plant)

%%
% The plant has an integrator, an unstable pole, and no zeros.

%%
% Display a Bode plot diagram
bode(plant)
...
% large (poor disturbance rejection) or too small (excessive computational
...
%%
% To achieve a more aggressive disturbance rejection, increase the state
...
%%
% The fact that the first column is close to zero shows that a constant 
% disturbance applied to the cart position (first output) is fully 
% recovered by the closed loop. On the other hand, the second column shows 
% that a constant disturbance applied to _theta_ passes unmitigated 
% through the closed loop and greatly affects the cart position (12.57). 
% In other words, while the closed loop is able to compensate for 
% a position disturbance (or equivalently track a position reference) it is
% not able to compensate for a constant disturbance in the measured angle
% (or equivalently track a constant reference angle).
