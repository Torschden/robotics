%% helper_functions.m - Supporting functions for sloshing controller
%
% These functions support the main sloshing analysis but are defined
% separately to keep main.m clean and organized.
...
function visualizeResults(results)
    ...
    for i = 1:n_scenarios
        if results{i}.success
            successful_results = [successful_results, i];
        end
    end
    
    n_successful = length(successful_results);
    if n_successful == 0
        fprintf('No successful optimizations to visualize.\n');
        return;
    end
    
    for idx = 1:n_successful
        i = successful_results(idx);
        
        % Plot trajectory and sloshing analysis
        subplot(2, n_successful, idx);
        ...
        subplot(2, n_successful, idx + n_successful);
    ...
    for i = 1:total_scenarios
        ...
        if results{i}.success
            ...
            if results{i}.sloshing_analysis.spill_risk
                fprintf('  WARNING: High spill risk detected!\n');
            else
